...
describe('Grist getMappingColumns', () => {
	const run = async (columns: unknown) => {
		const request = jest.fn().mockResolvedValue({ columns });
		...
		(loadOptions.getNodeParameter as jest.Mock).mockImplementation((name: string) => params[name]);
	...
	it('marks pure formula columns read-only but leaves trigger formulas writable', async () => {
		const { result } = await run([
		...
		expect(byId).toEqual({ formula: true, trigger: false, plain: false });
	});
